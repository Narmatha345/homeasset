...
**Live demo:** https://narmatha345.github.io/homeasset/ — installable as an app on mobile (Chrome: tap **Install app** in the menu; Safari: **Share → Add to Home Screen**).
...
The app deploys as two independent pieces: a static frontend on **GitHub Pages** and a live API on **Render** (free tier). GitHub Pages alone cannot run the Express/MongoDB backend, so both are needed for the hosted demo to actually work.

### 1. Backend → Render

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/narmatha345/homeasset)

This reads [`render.yaml`](render.yaml) and provisions a free web service (`npm install && npm run build` / `npm start`, health check at `/api/health`). It runs with `USE_MEMORY_DB=true`, so the demo data reseeds automatically on every cold start — no separate database to provision. `JWT_SECRET` is auto-generated by Render.

Once deployed, note the service URL Render gives you, e.g. `https://homeasset-api.onrender.com`.

### 2. Frontend → GitHub Pages

The [`deploy-pages` workflow](.github/workflows/deploy-pages.yml) builds `frontend/` and publishes it to GitHub Pages on every push to `main`. It reads the backend URL from a repository variable:

```bash
gh variable set VITE_API_URL --body "https://homeasset-api.onrender.com/api"
gh workflow run deploy-pages.yml
```

(First-time setup: in the repo's **Settings → Pages**, set the source to **GitHub Actions**.)

The site is served from `https://<your-username>.github.io/homeasset/` and is a full PWA — installable to a phone's home screen (Chrome: **Install app**; Safari: **Share → Add to Home Screen**) via the manifest and icons in `frontend/public/` and the `vite-plugin-pwa` config in `frontend/vite.config.ts`. Because it's a static host, the frontend uses `HashRouter` (URLs like `/#/dashboard`) instead of `BrowserRouter` so deep links and refreshes work without server-side rewrites.

**Note:** Render's free tier sleeps after 15 minutes of inactivity; the first request after a period of inactivity can take up to ~50 seconds while it wakes up (and, since it's an in-memory database, the demo data resets to the seeded state on every cold start).
